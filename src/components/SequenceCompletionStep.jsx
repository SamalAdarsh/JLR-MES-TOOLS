...
const SequenceCompletionStep = ({ 
  stepData, startTls, endTls, showConfirmModal, setShowConfirmModal, isSequencing, handleFinalSequence 
}) => {
  return (
    <>
      <div className={`bg-slate-900 rounded-2xl p-1 shadow-2xl transition-all duration-500 transform ${stepData['ag_processing'] && stepData['ag_processing'].result.length === 0 ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-8 scale-95 pointer-events-none'}`}>
        <div className="bg-linear-to-r from-slate-900 to-slate-800 rounded-xl p-8 text-center text-white border border-slate-700/50">
          <div className="mb-6 flex justify-center"><div className="p-3 bg-green-500/20 rounded-full text-green-400"><CheckCircle size={48} /></div></div>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4 animate-in fade-in duration-200">
          <div className="bg-[#1e2433] border border-slate-700/50 rounded-2xl shadow-2xl p-6 max-w-sm w-full text-center">
            <div className="w-16 h-16 bg-orange-500/10 rounded-full flex items-center justify-center mx-auto mb-4"><AlertTriangle className="text-orange-500 w-8 h-8" /></div>
            ...
            <div className="flex gap-3 w-full">
              <button onClick={() => setShowConfirmModal(false)} disabled={isSequencing} className="flex-1 px-4 py-2.5 bg-transparent border border-slate-600 hover:bg-slate-800 text-slate-300 font-semibold rounded-lg">Cancel</button>
              <button onClick={handleFinalSequence} disabled={isSequencing} className="flex-1 flex justify-center gap-2 px-4 py-2.5 bg-[#00b65e] hover:bg-[#009b50] text-white font-semibold rounded-lg">
