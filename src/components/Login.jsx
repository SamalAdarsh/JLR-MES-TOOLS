...
const Login = ({ onLogin, theme, darkMode, setDarkMode }) => {
  ...
  const handleSubmit = (e) => {
    ...
    // Simulate a secure network request for authentication
    setTimeout(() => {
      setIsLoading(false);
      onLogin(); // Proceed to main app on success
    }, 1500);
  };
      ...
      <div className={`w-full max-w-md p-8 rounded-3xl border shadow-2xl relative overflow-hidden transition-all duration-300 ${theme.card}`}>
        ...
        <div className="relative z-10">
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl font-extrabold bg-linear-to-r from-blue-500 to-indigo-600 bg-clip-text text-transparent mb-2">
              JLR MES TOOLS
            </h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button 
              ...
              className={`w-full py-3.5 mt-6 rounded-xl font-bold flex justify-center items-center gap-2 transition-all shadow-lg active:scale-95 ${
                ...
                : 'bg-linear-to-r from-blue-600 to-indigo-600 text-white hover:shadow-indigo-500/30 hover:from-blue-500 hover:to-indigo-500'
              }`}
            >
              {isLoading ? (
                <span className="animate-pulse flex items-center gap-2">Authenticating...</span>
              ) : (
